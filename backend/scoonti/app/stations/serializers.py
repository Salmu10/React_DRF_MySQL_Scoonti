...
class StationSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        total_scooters = len(Slot.objects.filter(station_id=instance.id, status='used'))
        return {
            "id": instance.id,
            "slug": instance.slug,
            "name": instance.name,
...
class SlotSerializer(serializers.ModelSerializer):
    ...
    def create(context):
        ...
        slot = Slot.objects.create(station_id=station_id, scooter_id=None, status="unused")
        slot.save()
        return slot
    ...
    def update(context, instance):
        ...
        if scooter_id != 0 and instance.scooter_id is not None:
            raise serializers.ValidationError('Slot is in use')

        if scooter_id != 0 and not None:
            scooter = Scooter.objects.get(pk=scooter_id)
            if scooter is None:
                raise serializers.ValidationError('Scooter is not find')
